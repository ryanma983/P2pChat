...
if ! java --module-path . --add-modules javafx.controls --version >/dev/null 2>&1; then
    echo "❌ JavaFX not available!"
    echo "Please install Java with JavaFX support or run ../check-javafx.sh"
    echo
    echo "Alternative: Use multi-cli-test.sh for command line version"
    exit 1
fi
...
case $choice in
    ...
    4)
        echo
        read -p "How many GUI nodes do you want to start (2-10)? " num_nodes
        
        if [ "$num_nodes" -lt 2 ] || [ "$num_nodes" -gt 10 ]; then
            echo "Invalid number. Please choose between 2-10."
            exit 1
        fi
